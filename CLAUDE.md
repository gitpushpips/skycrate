...
- **Jalon 1 — « ça vole »** *(en cours)*
  ...
  - [~] **Étape 5d** — feel & jouabilité (en cours). (A) **enveloppe élargie** : traînées baissées + poussée finie remontée ⇒ décollage ~30 m/s, vitesses bien plus hautes. (B) aéro/poussée/assistance déplacées en **pas fixe** (`useBeforePhysicsStep`) ⇒ boucle de contrôle stable (l'asservissement oscillait au rendu variable). (E) **assistance auto-steer** (`core/flight/assist.ts`, ON par défaut) : amortissement des taux tangage/roulis/lacet + **bornes d'attitude** (`maxPitch`/`maxBank`, gain `limitGain`). Au lâché, l'avion **GARDE son attitude/direction** (pas de remise à plat forcée) — `levelReturn` (ailes à plat) et `altHold` (maintien alt.) sont des **options leva à 0** par défaut. Calage d'aile ~2°. Reste : (C) bandes d'aile (amortissement roulis fin), (D) chute Cl post-décrochage + marge statique réglable.
...
- **Assistance (étape 5d-E)** : couple correctif **par-dessus** la physique (`core/flight/assist.ts`). Défaut = amortissement des taux + **bornes d'attitude** ⇒ au lâché, l'avion **garde sa direction** (demande utilisateur : NE PAS remettre à plat tout seul). `levelReturn`/`altHold` = options à 0. ⚠️ télémétrie `window.__plane` désormais **gardée mais `import.meta.env.DEV` only** (sera remplacée par le HUD étape 6).
